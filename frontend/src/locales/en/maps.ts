// frontend/src/locales/en/maps.ts
export default {
  searchPlaceholder: "Search...",
  searchAria: "Search map or realm",
  clearSearchAria: "Clear search input",
  providerAria: "Map Provider Source",
  all: "All",
  launchFullscreenEngine: "Launch Fullscreen Engine",
  zoomIn: "Zoom In",
  zoomInAria: "Zoom In",
  zoomOut: "Zoom Out",
  zoomOutAria: "Zoom Out",
  resetView: "Reset View",
  resetZoomPan: "Reset Zoom & Pan",
  resetPanZoom: "Reset Pan & Zoom",
  resetPanAndZoomAria: "Reset Pan and Zoom",
  fullscreenMode: "Fullscreen Interactive Mode",
  fullscreenAria: "Fullscreen Interactive Mode",
  popoutInWindow: "Popout Map in Window",
  noMapsFound: "No Maps Found",
  clockCalloutScheme: "12-Clock Callout Map System",
  mapVariantsAria: "Map Realm Variants",
  mapVariants: "Map Variants:",
  variants: "Variants:",
  engineControlsAria: "Engine Zoom and Reset Controls",
  fullscreenEngineAria: "2D Fullscreen Map Engine",
  voiceEngineAria: "Voice Map Navigation Engine",
  chromeEdgeSafari: "Chrome • Edge • Safari",
  universalPrivateInBrowser: "Universal • Private • In-Browser",
  preloadModel: "Preload Model",
  pageTitle: "LemonDBD - Tactical Map Command Explorer",
  initializingTacticalMap: "Initializing Tactical Map Command...",
  loadingTacticalMaps: "Loading Tactical Maps...",
  fit: "Fit",
  centerLandmarkObjective: "Center Landmark / Objective",
  sourceHensClock: "Hens333 (12-Clock)",
  sourceLemonDbd: "LemonDBD",
  lemonDbdSourceLocked: "Coming soon",
  sourceHens: "Hens333",
  detectedBrowser: "Detected Browser",
  activeRecognitionEngine: "Active Recognition Engine",
  bulletSeparator: "•",
  percentSign: "%",
  zoomPreset100: "100%",
  zoomPreset150: "150%",
  zoomPreset200: "200%",
  sourceLabel: "Source:",
  openQuote: "“",
  closeQuote: "”",
  heardLabel: "Heard:",
  keyV: "V",
  sectorCalloutsClockSystem: "Sector Callouts & Clock System",
  clockSystemSubtitle: 'Clockwise navigation callouts for team callouts',
  clockSystemTitle: 'Clockwise navigation callouts for team callouts',
  collapseLegend: 'Collapse Legend',
  diagramAlt: 'Diagram for {name}',
  diagramNotAvailable: 'Diagram for {name} is not yet available.',
  eastSector: '3 o\'clock / East Sector',
  expandLegend: 'Expand Legend',
  failedToLoadDiagram: 'Failed to load diagram for {name}. Please check network connection.',
  hens12ClockScheme: 'Hens333 12-Clock',
  nineOClock: '9 o\'clock',
  noDiagramAvailable: 'No diagram available',
  northSector: '12 o\'clock / North Sector',
  popoutPrefix: 'Popout',
  popoutSuffix: 'map in a new window',
  sixOClock: '6 o\'clock',
  southSector: '6 o\'clock / South Sector',
  threeOClock: '3 o\'clock',
  twelveOClock: '12 o\'clock',
  unableToLoadDiagram: 'Unable to load diagram',
  westSector: '9 o\'clock / West Sector',
  zoomIn25: 'Zoom In (+25%)',
  zoomOut25: 'Zoom Out (-25%)',
  searchTextTab: 'Search',
  searchVoiceTab: 'Voice',
  searchModeAria: 'Search mode',
  expandRealmAria: 'Expand realm',
  collapseRealmAria: 'Collapse realm',
  twelveClockCallouts: '12-Clock Callouts',
  tacticalDossier: 'Tactical Dossier',
  sqTilesUnit: 'sqT',
  sqMetersSuffix: '({value} m²)',
  palletsSuffix: '{density} Pallets',
  totemsSuffix: '{count} Totems',
  gymsSuffix: '{count} Gyms',
  shackBasementYes: 'Shack Basement',
  shackBasementNo: 'No Shack',
  surfaceArea: 'Surface Area',
  layoutLabel: 'Layout',
  palletDensityLabel: 'Pallet Density',
  mazeTilesLabel: 'Maze Tiles',
  dullTotemsLabel: 'Dull Totems',
  spawnsSuffix: '{count} Spawns',
  killerShackLabel: 'Killer Shack',
  corridorsLabel: '0 (Corridors)',
  basementPossible: 'Basement Possible',
  noShackBasement: 'No Shack Basement',
  specsAndIntelAria: 'Map Specifications and Intel',
  closeDossierAria: 'Close Dossier',
} as const;
