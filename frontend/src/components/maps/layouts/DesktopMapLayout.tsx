...
export const DesktopMapLayout: React.FC<DesktopMapLayoutProps> = ({
  ...
  const activeSourceLabel =
    activeMap?.source === 'samoelcolt'
      ? dict?.maps?.samoelSourceLabel || dict?.maps?.sourceSamoel || ''
      : dict?.maps?.hensSourceLabel || dict?.maps?.sourceHens || '';
  ...
  return (
    ...
    >
      <aside
        className={`shrink-0 flex flex-col rounded-3xl border border-slate-200/90 dark:border-slate-800/90 bg-white/95 dark:bg-slate-900/95 shadow-lg dark:shadow-2xl backdrop-blur-xl overflow-hidden transition-all duration-300 ${isSidebarCollapsed ? 'w-14' : 'w-full lg:w-80 xl:w-[22rem]'
          }`}
        data-testid="desktop-map-sidebar"
        aria-label={dict?.maps?.mapDirectory || ''}
      >
        <div className="p-3.5 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between gap-2 shrink-0 bg-slate-50/50 dark:bg-slate-950/40">
          ...
          <button
            ...
            aria-label={
              isSidebarCollapsed
                ? dict?.maps?.expandSidebar || ''
                : dict?.maps?.collapseSidebar || ''
            }
            title={
              isSidebarCollapsed
                ? dict?.maps?.expandSidebar || ''
        ...
        {!isSidebarCollapsed && (
          <>
            <div className="p-3 border-b border-slate-200 dark:border-slate-800/80 space-y-2.5 shrink-0">
              <div className="relative w-full">
                ...
                {search && (
                  <button
                    ...
                    aria-label={dict?.maps?.clearSearchAria || ''}
                    className="absolute right-2.5 top-2.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-0.5 rounded-md focus:outline-none focus:ring-1 focus:ring-slate-400"
                  >
              ...
              >
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 rounded-lg py-1 text-[11px] font-extrabold transition-all cursor-pointer min-h-[30px] focus:outline-none focus-visible:ring-1 focus-visible:ring-cyan-500 ${activeSource === 'hens333'
                ...
                >
                  ...
                  <span>{dict?.maps?.sourceHens || ''}</span>
                </button>
                <button
                  type="button"
                  onClick={() => onSourceChange('samoelcolt')}
                  aria-pressed={activeSource === 'samoelcolt'}
                  className={`flex-1 flex items-center justify-center gap-1.5 rounded-lg py-1 text-[11px] font-extrabold transition-all cursor-pointer min-h-[30px] focus:outline-none focus-visible:ring-1 focus-visible:ring-purple-500 ${activeSource === 'samoelcolt'
                ...
                >
                  ...
                  <span>{dict?.maps?.sourceSamoel || ''}</span>
                </button>
                <button
                  type="button"
                  onClick={() => onSourceChange('all')}
                  aria-pressed={activeSource === 'all'}
                  className={`flex-1 flex items-center justify-center gap-1.5 rounded-lg py-1 text-[11px] font-extrabold transition-all cursor-pointer min-h-[30px] focus:outline-none focus-visible:ring-1 focus-visible:ring-slate-500 ${activeSource === 'all'
            ...
            <div
              role="group"
              aria-label={dict?.maps?.realmFiltersAria || ''}
            ...
            >
              <button
                ...
                className={`px-2.5 py-1 rounded-xl text-[11px] font-black whitespace-nowrap transition-all cursor-pointer border min-h-[28px] focus:outline-none focus-visible:ring-1 focus-visible:ring-cyan-500 ${selectedRealm === 'all'
              ...
              {uniqueRealms.map((r) => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-xl text-[11px] font-bold whitespace-nowrap transition-all cursor-pointer border min-h-[28px] focus:outline-none focus-visible:ring-1 focus-visible:ring-cyan-500 ${selectedRealm === r
                        ? 'bg-cyan-500/15 border-cyan-500/40 text-cyan-700 dark:text-cyan-300 shadow-sm'
                        : 'bg-white border-slate-200 text-slate-600 hover:text-slate-900 hover:border-slate-300 dark:bg-slate-950 dark:border-slate-800 dark:text-slate-400 dark:hover:text-white dark:hover:border-slate-700'
                      }`}
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="rounded-full bg-cyan-500/10 border border-cyan-500/30 px-3 py-1 text-xs font-black text-cyan-700 dark:text-cyan-400 whitespace-nowrap font-mono">
              {activeMap?.realm || dict?.maps?.selectMap || ''}
            </span>
            <span
              className={`hidden sm:inline-block rounded-full border px-2.5 py-0.5 text-[10px] font-black uppercase whitespace-nowrap font-mono ${activeMap?.source === 'samoelcolt'
            ...
            <h2 className="text-base sm:text-lg font-black text-slate-900 dark:text-white truncate font-mono tracking-tight">
              {activeMap ? activeMap.name : dict?.maps?.noMapSelected || ''}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {onLaunchFullscreen && (
              <button
                ...
                aria-label={dict?.maps?.launch2DEngine || ''}
                ...
                title={dict?.maps?.launch2DEngine || ''}
              >
                <Maximize2 className="h-3.5 w-3.5" />
                <span className="hidden sm:inline">{dict?.maps?.twoDEngine || ''}</span>
              </button>
            )}
            {onPopoutImage && activeMap && (
              <button
                ...
                aria-label={dict?.maps?.popoutAria || ''}
                ...
                title={dict?.maps?.popoutAria || ''}
              >
                <ExternalLink className="h-3.5 w-3.5" />
                <span className="hidden sm:inline">{dict?.maps?.popout || ''}</span>
              </button>
            )}
          </div>
        </header>
