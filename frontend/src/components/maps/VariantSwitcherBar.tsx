        ...
        {variants.map((v) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold transition-all cursor-pointer min-h-[36px] focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-red ${
                isActive
                  ? 'bg-accent-red text-text-inverted shadow-md font-extrabold scale-105'
                  : 'bg-bg-surface border border-border-color text-text-secondary hover:border-accent-red/60 hover:text-text-primary shadow-sm'
              }`}
            ...
            >
              <span>{v}</span>
              {isActive && (
                <Check className="h-3.5 w-3.5 text-text-inverted" data-testid="variant-active-check" />
              )}
            </button>
          );
        })}
