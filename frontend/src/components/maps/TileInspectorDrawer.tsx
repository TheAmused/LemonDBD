...
export const TileInspectorDrawer: React.FC<TileInspectorDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full sm:w-96 bg-slate-900/95 border-l border-slate-800 shadow-2xl backdrop-blur-xl z-[60] flex flex-col justify-between transition-transform duration-300 ease-in-out">
      {/* Header */}
      <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-950/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs font-mono uppercase tracking-wider text-amber-400/90">{itemType}</span>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="p-5 space-y-5 overflow-y-auto flex-1 scrollbar-thin">
        ...
        {(hasWindow || vaultDirections) && (
          <div className="p-4 bg-slate-950/80 border border-indigo-500/30 rounded-2xl space-y-2.5">
            ...
            {vaultDirections && (
              <div className="text-xs font-medium text-slate-300 flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 bg-indigo-950 border border-indigo-500/40 text-indigo-300 rounded font-mono text-[11px]">
                  {vaultDirections}
                </span>
              </div>
            )}
            <p className="text-xs text-slate-300 leading-relaxed">
              ⚡ <strong className="text-indigo-200">Fast Vault (0.5s):</strong> Requires running straight at window with at least 2.5m momentum.
              <br />
              🏃 <strong className="text-slate-400">Medium Vault (0.9s):</strong> Triggers on angled approach. High risk of killer hit!
            </p>
          </div>
        )}
        ...
        <div className="p-4 bg-slate-950/80 border border-red-500/30 rounded-2xl space-y-2">
        ...
        {locationDesc && (
          <div className="p-3 bg-slate-950/40 border border-slate-800 rounded-xl text-xs text-slate-400 flex items-center justify-between">
            <span className="text-slate-500">Location Note:</span>
            <span className="text-slate-200 font-medium text-right">{locationDesc}</span>
          </div>
        )}
      </div>
