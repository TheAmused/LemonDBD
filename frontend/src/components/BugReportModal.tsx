...
export const BugReportModal: React.FC<BugReportModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!message.trim()) {
            setErrorMsg('Please describe the bug or issue in detail.');
            return;
        }
        if (!isAuthenticated && !guestEmail.trim()) {
            setErrorMsg('Please enter your email so we can follow up with you.');
            return;
        }
        ...
        try {
            ...
            if (contentType.includes('application/json')) {
                ...
                data = { error: `Server returned status ${res.status}: ${textResp.slice(0, 100)}` };
            }

            if (!res.ok) {
                setErrorMsg(data.error || `Server returned error (${res.status}).`);
            } else {
                setIsSuccess(true);
            }
        } catch (err: unknown) {
    ...
    return (
        ...
        @keyframes entityGlowAura {
          0%, 100% {
            box-shadow: 0 0 25px 1px rgba(220, 38, 38, 0.25), inset 0 0 15px rgba(220, 38, 38, 0.08);
          }
          50% {
            box-shadow: 0 0 45px 5px rgba(220, 38, 38, 0.45), inset 0 0 25px rgba(220, 38, 38, 0.18);
          }
        }
      `}</style>
            ...
            >
                ...
                >
                    {/* Subtle Blood-Red Top Gradient Bar */}
                        ...
                        <div className="text-center py-8 space-y-5">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <p className="text-xs text-slate-400 max-w-sm mx-auto">
                                    The Entity has relayed your report to the developer Discord server. You can track updates under your Profile subtab!
                                </p>
                            </div>
                            <div className="pt-3">
                                <button
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-3.5 border-b border-rose-950/60 pb-4">
                                ...
                                <div>
                                    ...
                                    >
                                        <span>Report a Glitch or Bug</span>
                                        <Flame className="h-4 w-4 text-rose-500" />
                                    </h2>
                                    <p className="text-xs text-slate-400">
                                        Transmits directly to developer Discord webhook
                                    </p>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                {isAuthenticated && user ? (
                                    <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/60 p-3 text-xs">
                                        <div className="flex items-center gap-2.5 text-slate-300">
                                            ...
                                            <div>
                                                <p className="font-bold text-slate-200">Reporting as {user.username}</p>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <div className="sm:col-span-2">
                                        ...
                                        <input
                                            ...
                                            placeholder="e.g. Adrenaline stats incorrect"
                                            className="w-full rounded-xl border border-slate-800 bg-slate-900/80 px-3.5 py-2.5 text-xs text-slate-100 placeholder-slate-500 focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500 transition-all shadow-inner"
                                        />
                                    </div>
                                ...
                                <div>
                                    <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">
                                        Detailed Description & Steps <span className="text-rose-500">*</span>
                                    </label>
                                    <textarea
                                        required
                                        rows={4}
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-1.5">
                                        <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                                            ...
                                            Screenshots / Evidence (Max 3)
                                        </label>
                                        <span className="text-[10px] text-slate-500">{images.length}/3 attached</span>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-3">
                                        {images.map((imgSrc, idx) => (
                                            ...
                                            >
                                                <img src={imgSrc} alt={`Attachment ${idx + 1}`} className="h-full w-full object-cover" />
                                                <button
                                                    type="button"
