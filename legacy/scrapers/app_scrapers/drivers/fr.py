# legacy/scrapers/app_scrapers/drivers/fr.py
# backend/app/scrapers/drivers/fr.py
...
class WikiGGDriverFR(BaseWikiDriver):
    """Specialized driver for scraping Dead by Daylight French Wiki (fr.deadbydaylight.wiki.gg)."""

    def __init__(self, base_dir: Optional[Any] = None):
        super().__init__(base_dir=base_dir, lang_code="fr")
    ...
    def enrich_translations(
        ...
        """Scrapes French pages and enriches canonical entities with French names and descriptions."""
        from app.scrapers.drivers.base import PORTRAIT_PATTERN, extract_icon_token

        logger.info("Enriching entities with French (fr) translations...")
        ...
        # 1. French Perks (Compétences)
        try:
            p_html = self.fetch_page_html("Compétences")
            if p_html:
                    ...
                    for row in table.find_all("tr")[1:]:
                        ...
                        if len(cells) >= 2:
                            ...
                            if matched_p and translated_name and not translated_name.isdigit():
                                clean_desc = desc if desc else (matched_p.translations.get("fr", {}).get("description") or "")
                                matched_p.translations["fr"] = {
                                    "name": translated_name,
                                    "description": clean_desc,
                                }
        except Exception as e:
            logger.warning(f"Error enriching French perks: {e}")

        # 2. French Characters (Tueurs and Survivants)
        try:
            for c_page in ["Tueurs", "Survivants"]:
                                ...
                                for img in imgs:
                                    ...
                                    if m:
                                        ...
                                        if matched_c:
                                            ...
                                            if translated_name:
                                                if "fr" not in matched_c.translations:
                                                    matched_c.translations["fr"] = {
                                                        "name": translated_name,
                                                        "lore": matched_c.lore or "",
                                                    }
                                                else:
                                                    matched_c.translations["fr"]["name"] = translated_name
        except Exception as e:
            logger.warning(f"Error enriching French characters: {e}")

        # 3. French Items (Objets)
        try:
            i_html = self.fetch_page_html("Objets")
            if i_html:
                    ...
                    for row in table.find_all("tr")[1:]:
                        ...
                        if len(cells) >= 2:
                            ...
                            if matched_i and name:
                                matched_i.translations["fr"] = {
                                    "name": name,
                                    "description": desc,
                                }
        except Exception as e:
            logger.warning(f"Error enriching French items: {e}")

        # 4. French Addons (Accessoires)
        try:
            a_html = self.fetch_page_html("Accessoires")
            if a_html:
                    ...
                    for row in table.find_all("tr")[1:]:
                        ...
                        if len(cells) >= 2:
                            ...
                            if matched_a and name:
                                matched_a.translations["fr"] = {
                                    "name": name,
                                    "description": desc,
                                }
        except Exception as e:
            logger.warning(f"Error enriching French addons: {e}")
