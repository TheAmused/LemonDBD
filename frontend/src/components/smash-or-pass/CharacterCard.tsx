  ...
  return (
    <>
      <div
        ...
        className={`relative select-none w-[86vw] max-w-[340px] sm:max-w-[380px] md:max-w-[420px] aspect-[9/14] sm:aspect-[9/15] rounded-[32px] sm:rounded-[36px] overflow-hidden border-2 cursor-grab active:cursor-grabbing shadow-2xl transition-all duration-300 ${
          isSwipingRight
      ...
      >
        ...
        <div
          className="pointer-events-none absolute inset-0 z-20 opacity-30 mix-blend-overlay transition-opacity"
          style={{
        ...
        {/* 3D FLIPPABLE CONTAINER WITH DISSOLVE EFFECT ON IMAGE/BODY ONLY */}
        <div
          style={{
        ...
        >
          ...
          >
            ...
            <div className="absolute inset-0 z-0 bg-slate-950 overflow-hidden">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-b from-slate-950/60 via-transparent to-slate-950/75 pointer-events-none" />
            </div>

            {/* TOP CONTROLS BAR (ICON ONLY) */}
            ...
            <div className="relative z-30 flex items-center justify-between p-3.5 sm:p-4">
              {/* Bottom-Left: PASS BUTTON (Hands Down / Thumbs Down) */}
              <button
                type="button"
                onMouseDown={(e) => e.stopPropagation()}
                onTouchStart={(e) => e.stopPropagation()}
              ...
              {/* Bottom-Right: SMASH BUTTON (Heart) */}
              <button
                type="button"
                onMouseDown={(e) => e.stopPropagation()}
                onTouchStart={(e) => e.stopPropagation()}
                ...
                title="Smash"
                className="flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl bg-gradient-to-r from-rose-600 to-pink-600 text-white hover:scale-110 active:scale-95 transition-all shadow-2xl shadow-rose-950/60 border-2 border-pink-400/40 backdrop-blur-md cursor-pointer"
              >
                <Heart className="h-6 w-6 sm:h-7 sm:w-7 fill-white animate-pulse" />
              </button>
            </div>
          </div>

          {/* ================= CARD BACK (DATING PROFILE - ZERO TEXT ON BUTTONS) ================= */}
          <div
            style={{
              backfaceVisibility: 'hidden',
              transform: 'rotateY(180deg)',
            }}
            className={`absolute inset-0 h-full w-full flex flex-col justify-between p-4 sm:p-5 text-left bg-slate-950/95 backdrop-blur-2xl space-y-2.5 overflow-y-auto transition-opacity duration-300 ${
            ...
            <div className="grid grid-cols-1 gap-1.5 sm:gap-2">
              <div className="space-y-0.5 sm:space-y-1 bg-emerald-950/30 border border-emerald-500/20 p-2.5 rounded-2xl">
                ...
                <ul className="text-xs text-emerald-200/90 space-y-0.5 pl-4 list-disc">
                  {character.greenFlags.map((flag, idx) => (
                    <li key={idx}>{flag}</li>
                  ))}
                </ul>
              </div>
              ...
              <div className="space-y-0.5 sm:space-y-1 bg-rose-950/30 border border-rose-500/20 p-2.5 rounded-2xl">
                ...
                <ul className="text-xs text-rose-200/90 space-y-0.5 pl-4 list-disc">
                  {character.redFlags.map((flag, idx) => (
                    <li key={idx}>{flag}</li>
                  ))}
                </ul>
              </div>
