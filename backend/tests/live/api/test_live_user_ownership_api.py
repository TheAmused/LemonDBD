...
@pytest.mark.live
class TestLiveUserOwnershipAPI:
    """Tests for User profile retrieval, character ownership toggling, and perk unlocks on live database."""

    def test_live_user_registration_login_and_ownership(
        self,
        ...
        client, headers, user = auth_client_factory(
            "live_user_1", "live_1@example.com", "pass123"
        )
        user_id = user["id"]
    ...
    def test_live_perk_ownership_toggle(
        ...
        client, headers, user = auth_client_factory(
            "live_user_perks", "live_p@example.com", "pass123"
        )
        user_id = user["id"]
